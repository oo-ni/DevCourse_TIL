...
public class Test01_순열 {
    ...
    static String[] result = new String[R];     // N개 중에 R개를 뽑아서 순서까지 고려해서 채워놓을 배열.
    ...
    static void perm(int idx) {         // 함수 한놈한테 줘야하는 재료가 무엇인지.
        ...
        for (int i = 0; i < N; i++) {   // 재귀가 한번 호출되면 A부터 E까지 모든 카드를 한번씩 다 보고 끝남.
            ...
            result[idx] = cards[i];     // 먼저 호출된 재귀가 안쓰는 카드네? 내가 맡은 자리 idx에다 i번 카드를 써볼게요.
            used[i] = true;             // i번 카드 내가 쓰고있다고 내 다음 재귀들한테 알려야 해요!
            perm(idx + 1);          // 자 이제 얘가 스택에서 나갈 때까지 나는 이 아래 문장 실행 안하고 계속 기다릴거임!!
            used[i] = false;
        }
    }
