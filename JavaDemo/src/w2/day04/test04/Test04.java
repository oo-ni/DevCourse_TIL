package w2.day04.test04;

import java.util.Arrays;
import java.util.Comparator;

public class Test04 {
    private static int haha = 100;
    public static void main(String[] args) {
        ...
        students[7] = new Student("이성원", 25, 85);
        students[8] = new Student("양아영", 20, 90);

        Arrays.sort(students, (o1, o2) -> {
            int num = 100;      // 람다도 하나의 메소드라서 안에서 필요한 변수 만들어 쓰는건 지장 없음.
            System.out.println("이름으로 정렬중!");
//            students = new Student[10];       // 정렬하는 도중에 잠깐잠간 쓰는 부품함수 주제에 밖에 있는 데이터를 변경해? 안돼ㅐㅇ애애ㅐ!!!!!!!
            haha = 200;         // 밖에 있는 지역변수는 건드리면 안됨.. 근데 클래스 멤버야? 그러면 객체 앞에 계속 유지되는 변수네? 그럼 건드릴 수 있음!
            // 람다 내부에서 외부의 지역 변수를 변경하려고 하면 컴파일 에러가 발생함.
        ...
        /**
         * 람다는 무명 클래스로 메소드 하나짜리객체를 매번 정의해야 하는 상황에서 태어났음.
         * 기본 문법은 (매개변수) -> {메소드 실행문} 이지만,
