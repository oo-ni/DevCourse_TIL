...
class MyJwtTokenProvider {
    ...
    fun createMyToken(entity: UserEntity, expireTime: Long) : String {
        return Jwts.builder()
            ...
            .issuer("geonoo company")   // 토큰 발급처
            .issuedAt(Date())           // 토큰 생성 시간
            .expiration(Date(System.currentTimeMillis() + expireTime))  // 발급 시간기준 얼마나 오래 유효한지
            .compact()
    }

    // 나중에 사용자가 토큰을 들고왔네? 토큰 유효성 검증!
