...
public class ForwardController {
    ...
    public ModelAndView mav() {
        ...
        mav.setViewName("view1"); // viewResolver를 거쳐서 /WEB-INF/views/view1.jsp 라는 뷰 경로가 완성됨.
        mav.addObject("msg", "ForwardController가 view에게 전달하는 데이터");
        mav.addObject("now", new Date());
        return mav; // 데이터와 그 데이터를 전달받을 뷰의 이름이 모두 들어있음
    }

    // return 되는 String을 뷰 이름으로 하고 전달받은 파라미터 DTO 객체와 추가로 데이터를 더 담은 model의 gugudan들도 모듀 view에게 전할 수 있음.
    @RequestMapping("/str")
