...
//  }
    ...
    public BoardDTO read(int bno) throws SQLException {
        BoardDTO boardDTO = boardRepo.selectOne(bno);       // board_tb에서 조회
...
//    }
    ...
    @Transactional
    // 현재 보고자 하는 페이지 정보가 들어왔을 때, 실제 해당 페이지에 보여져야 하는 List<BoardDTO>를 포함해서 페이지가 총 몇 개 필요하고, 하단 페이지 링크는 1-10 or 11-20 같은 페이지 구간 계산
    public Map<String, Object> getBoards(int page) {
        int totalCount = boardRepo.selectCount();                // 총 게시글의 갯수 (ex : 27)
        ...
        int endPage = startPage + 9;                // 보통 화면 하단 시작페이지가 11이면 끝은 20, 21이면 3에서 끝나던데?
        if (totalPageCount < endPage) {         // 21~30 이라고 계산했는데 총 페이지수가 28밖에 없었다? 페이지 너무 많다고 뻥치고 있네?
        ...
        List<BoardDTO> boardList = boardRepo.selectList(startRow, COUNT_PER_PAGE);
        ///////////////////////// Service는 이렇게 여러가지 비즈니스 로직을 수행해서 데이터를 계산해 냄.
    ...
    public int saveFileInfo(List<FileDTO> fileDTOList, int bno) {   // 작성된 글 하나에 파일이 여러 개 첨부될 수 있음.
        if (fileDTOList == null || fileDTOList.isEmpty()) return 0;

        for (FileDTO f : fileDTOList) {     // 파일 이름 originalName, 저장된 경로 savedPath만 설정되어 있으니까 게시글 번호 붙여서 insert 시켜야 됨!
            f.setBno(bno);
        }
