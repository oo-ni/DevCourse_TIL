...
@Component
public class GeonooFilter extends OncePerRequestFilter {    // request요청 한 번 당 처리해주는 필터. Filter는 DispatcherServlet보다 먼저 작업을 처리함.
    @Autowired
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            System.out.println("starat geonoo filter : " + token);      // 이런거 찍으면 안됨 원래...
            if (token != null) {
                ...
                AbstractAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(
                        username,   // 지금처럼 문자열 말고 다른 것도 들어갈 수 있는데, 예제에서는 UserDetails 라는 오브젝트를 넣는 경우가 많음. 우리는 안했음.
                        null,
                        AuthorityUtils.NO_AUTHORITIES   // 로그인이 된 상태에서도 관리자, 일반 사용자 등의 권한이 달라질 수 있음. 여기서 설정.
                );
                // 토큰에다가 기본적인 인증정보 외에 더 부가적으로 달아놓고 싶은 내용이 있으면 아래 Detail에서 더 담는 작업 수행 가능
    ...
    // 저 Bearer라는 글자가 기능적인 역할은 없는데 토큰 개발하는 프론트 쪽에서 관습적으로 항상 붙이는거 같은...
